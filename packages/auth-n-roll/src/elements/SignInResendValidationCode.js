...
import { getResendValidationCodeSendingState, getResendValidationCodeSendingError } from '../store/selectors'
...
SignInResendValidationCode.MessageSending = ({ children }) => (
  <StateFilter
    ...
    value={RESEND_CONFIRMATION_CODE_STATE_SENDING}
...
SignInResendValidationCode.MessageSendingSuccess = ({ children }) => (
  <StateFilter
    getStateFunction={getResendValidationCodeSendingState}
    value={RESEND_CONFIRMATION_CODE_STATE_SENDING_SUCCESS}
  >{children}</StateFilter>
)
...
SignInResendValidationCode.MessageSendingError = ({ children }) => (
  <StateFilter
    getStateFunction={getResendValidationCodeSendingState}
    value={RESEND_CONFIRMATION_CODE_STATE_SENDING_ERROR}
  >{children}</StateFilter>
)
    ...
    <React.Fragment>
      {React.Children.map(props.children, child =>
        React.cloneElement(child, {
          ...
          disabled: getResendValidationCodeSendingState(props.authNRoll) === RESEND_CONFIRMATION_CODE_STATE_SENDING
        })
      )}
    </React.Fragment>
  )
